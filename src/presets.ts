...
export const PRESETS: Record<string, DomainConfig> = {
  generic: {
    name: "generic",
    curatedTerms: {
 ...
 */
export function detectDomainPreset(text: string): keyof typeof PRESETS {
  ...
  for (const word of gridWords) {
    if (lowercaseText.includes(word)) indicators.smartGrid += 1.5;
  }
  const gridRegexes = [/\bieee\s+\d+\b/i, /\bsep\s*2\b/i, /\bvoltage\b/i];
  for (const regex of gridRegexes) {
    if (regex.test(lowercaseText)) indicators.smartGrid += 1.0;
  }
  ...
  for (const word of securityWords) {
    if (lowercaseText.includes(word)) indicators.security += 1.5;
  }
  const securityRegexes = [/\baccess\s+control\b/i, /\bsecure\s+transport\b/i];
  for (const regex of securityRegexes) {
    if (regex.test(lowercaseText)) indicators.security += 1.0;
  }

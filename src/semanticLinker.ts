...
export class SemanticLinker {
  ...
  constructor(topNTerms = 20, config?: { curatedTerms?: Record<string, RegExp>; stopwords?: string[]; actors?: string[] }) {
  ...
  async buildKnowledgeGraph(ruleLedger: Rule[], blocks: Block[], ctx?: any, signal?: AbortSignal): Promise<KnowledgeGraph> {
    const nodes: KGNode[] = [];
    const edges: KGEdge[] = [];
    ...
    let extractedTerms: Record<string, number> = {};
    if (ctx) {
    ...
    const candidateConflicts: Array<{ r1: Rule; r2: Rule; term: string }> = [];
...
Respond with valid JSON ONLY. Do not wrap in markdown or backticks.`;
  ...
  async extractTermsLLM(
    ...
    for (let i = 0; i < ruleLedger.length; i += step) {
      ...
      if (sampleRules.length >= sampleSize) break;
    }
...
Respond with valid JSON ONLY. Do not wrap in markdown or backticks.`;
      ...
      if (text) {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          ...
          for (const term of parsed) {
            ...
            if (commonActors.includes(termClean)) continue;
  ...
  async validateConflictsWithLLM(
    candidates: Array<{ r1: Rule; r2: Rule; term: string }>,
    ctx: any,
    signal?: AbortSignal
  ): Promise<Array<{ r1: Rule; r2: Rule; term: string; reason: string }>> {
    const verified: Array<{ r1: Rule; r2: Rule; term: string; reason: string }> = [];

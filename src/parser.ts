...
export async function validateHeadingsWithLLM(
  candidates: { id: number; line: string }[],
  ctx: any,
  signal?: AbortSignal
): Promise<Set<number>> {
  const falseHeadingIds = new Set<number>();
  if (candidates.length === 0) return falseHeadingIds;
...
export class PDFParser {
  ...
  async parse(signal?: AbortSignal, ctx?: any): Promise<Block[]> {
    const rawLinesInfo: Array<[number, string, string, any, number]> = [];
    const candidateHeadings: { id: number; line: string }[] = [];
    ...
    let falseHeadingIds = new Set<number>();
    ...
    }) as Array<[number, string, string, any]>;

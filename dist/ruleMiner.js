...
export class RuleMiner {
            ...
            for (const sentence of sentences) {
                try {
                    ...
                    if (matchedConstraints.length > 0) {
                        ...
                        let dominantType = "Permission";
                        for (const cType of ["Prohibition", "Mandatory", "Recommendation"]) {
    ...
    async mineImplicitRules(blocks, existingRules, ctx, signal) {
        const matchedBlockIds = new Set(existingRules.map(r => r.source_block_id));
        const candidateBlocks = blocks.filter(b => {
            if (b.type === "heading" || b.type === "table")
                return false;
            if (matchedBlockIds.has(b.id))
                return false;
...
Respond with valid JSON ONLY. Do not wrap in markdown or backticks.`;
    ...
    async mineTabularRules(blocks, ctx, signal) {
        if (!ctx)
            return [];
...
export async function filterBoilerplateRules(rules, ctx, signal) {
    if (rules.length === 0 || !ctx)
        return rules;
    const boilerplateIds = new Set();
    const batchSize = 100;
